 ...
 */
	...
	if (!$data = XoopsCache::read($cachekey = _MD_FONTIER_MODULE_DIRNAME . "-preview-".md5($_GET['id'].$_GET['format'])) && !(isset($data['image']) && !empty($data['image'])))
	{
		// API Load Balancing
		...
		// Calls API
		$data = array('image'=>getURIData(str_replace("%apipath%", $fontierConfigsList['api_path'], str_replace("%identity%", $font->getVar('identity'), str_replace("%format%", $_GET['format'], $fontierConfigsList['api_path_preview'])))));
		if (!empty($data) && $fontierConfigsList['cache_preview'] > 0)
		{
			XoopsCache::write($cachekey, $data, $fontierConfigsList['cache_preview']);
			$caches = XoopsCache::read(_MD_FONTIER_MODULE_DIRNAME . "-caches-session");
			if (!is_array($caches))
				$caches = array();
			$caches[$cachekey] = time() + $fontierConfigsList['cache_preview'];
